...
public class ZipTemplateCommand extends BaseCommand {
    public ZipTemplateCommand() {
        super("zipTemplate", b -> b.executes(ctx -> {
            var storage = InjectionLayer.boot().instance(TemplateStorageProvider.class).templateStorage("woolbattle");
            System.out.println("Starting");
            storage.zipTemplateAsync(ServiceTemplate.builder().storage("woolbattle").prefix("test").name("default").build()).whenComplete((zipInputStream, throwable) -> {
